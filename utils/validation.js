...
export function validateFoodItem(data) {
  ...
  if (!data.name || typeof data.name !== 'string' || data.name.trim().length === 0) {
    ...
    if (!VALIDATION_RULES.ITEM_NAME.PATTERN.test(trimmedName)) {
      errors.push('Item name must contain only letters, numbers, spaces, hyphens, parentheses, and ampersands');
    }
    if (trimmedName.length < VALIDATION_RULES.ITEM_NAME.MIN_LENGTH) {
      errors.push(`Item name must be at least ${VALIDATION_RULES.ITEM_NAME.MIN_LENGTH} characters long`);
    }
    if (trimmedName.length > VALIDATION_RULES.ITEM_NAME.MAX_LENGTH) {
      errors.push(`Item name must be less than ${VALIDATION_RULES.ITEM_NAME.MAX_LENGTH} characters`);
    }
  }
  ...
  if (!data.unit || typeof data.unit !== 'string' || data.unit.trim().length === 0) {
    ...
    if (trimmedUnit.length < VALIDATION_RULES.UNIT.MIN_LENGTH) {
      errors.push(`Unit must be at least ${VALIDATION_RULES.UNIT.MIN_LENGTH} character long`);
    }
    if (trimmedUnit.length > VALIDATION_RULES.UNIT.MAX_LENGTH) {
      errors.push(`Unit must be less than ${VALIDATION_RULES.UNIT.MAX_LENGTH} characters`);
    }
  }
  ...
  if (data.notes && typeof data.notes === 'string' && data.notes.trim().length > 0) {
    ...
    if (trimmedNotes.length > VALIDATION_RULES.NOTES.MAX_LENGTH) {
      errors.push(`Notes must be less than ${VALIDATION_RULES.NOTES.MAX_LENGTH} characters`);
    }
  }
...
export function validateCategory(data) {
  ...
  if (!data.name || typeof data.name !== 'string' || data.name.trim().length === 0) {
    ...
    if (!VALIDATION_RULES.CATEGORY_NAME.PATTERN.test(trimmedName)) {
      errors.push('Category name must contain only letters, spaces, and hyphens');
    }
    if (trimmedName.length < VALIDATION_RULES.CATEGORY_NAME.MIN_LENGTH) {
      errors.push(`Category name must be at least ${VALIDATION_RULES.CATEGORY_NAME.MIN_LENGTH} characters long`);
    }
    if (trimmedName.length > VALIDATION_RULES.CATEGORY_NAME.MAX_LENGTH) {
      errors.push(`Category name must be less than ${VALIDATION_RULES.CATEGORY_NAME.MAX_LENGTH} characters`);
    }
  }
